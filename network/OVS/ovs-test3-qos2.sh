...
# 增加一个 veth peer 直接连接两个网桥, 方式一：
	...
	sudo ip link del name vlanbrside type veth peer name qosbrside 
	sudo ip link add name vlanbrside type veth peer name qosbrside # default MTU 是多少
...
: "
# 连接同一个网段的两个网桥， ChatGPT帮忙，方式二：用这个方式 设置QOS没有成功，可能是参数没搞对
...
sudo ovs-vsctl set port vlanbrside qos=@newqos \
-- --id=@newqos create qos type=linux-htb other-config:max-rate=100000000 queues=0=@q0,1=@q1,2=@q2 \
-- --id=@q0 create queue other-config:min-rate=30000000 other-config:max-rate=100000000 \
-- --id=@q1 create queue other-config:min-rate=10000000 other-config:max-rate=100000000 \
-- --id=@q2 create queue other-config:min-rate=60000000 other-config:max-rate=100000000

# tc -s -d class show dev vlanbrside
# class htb 1:fffe root rate 10Mbit ceil 10Mbit linklayer ethernet burst 1500b/1 mpu 60b cburst 1500b/1 mpu 60b level 7 
#  1:fffe  为入口队列
...
sudo ovs-vsctl set Interface qosbrside ingress_policing_rate=0
sudo ovs-vsctl set Interface qosbrside ingress_policing_burst=0


# 10000 -> 1M  100000 -> 2M 左右  1000000 -> 300M了，好像不是很准呢， 参考上面单位备注
sudo ovs-vsctl set Interface qosbrside ingress_policing_rate=10000
sudo ovs-vsctl set Interface qosbrside ingress_policing_burst=1000

## 实验完毕，清理
#sudo ovs-vsctl set Interface qosbrside  ingress_policing_burst=0
#sudo ovs-vsctl set Interface qosbrside  ingress_policing_rate=0
# sudo ovs-vsctl list Interface qosbrside


## 规则配置完毕， 可以测试了
# 从各个实例上访问 103   netperf是网络性能测试工具, 流量有限制住
# UDP_STREAM 结果有两行测试数据：
# 第一行显示的是发送端的流量统计 client -> server ，第二行是远端回程流量数据统计 server -> client
# !image(./netpert-UDP_STREAM-result.jpg)
# 参考： https://www.cnblogs.com/ykhyq/p/9634522.html


sudo docker exec -it h100 netperf -H 192.168.100.106 -t UDP_STREAM  -b 100M
sudo docker exec -it h101 netperf -H 192.168.100.106 -t UDP_STREAM  -b 100M
sudo docker exec -it h102 netperf -H 192.168.100.106 -t UDP_STREAM  -b 100M
...
sudo ovs-vsctl list Port vlanbrside
sudo ovs-vsctl clear Port vlanbrside qos #清除后应该QOS为空， 要先删除qos和queue,再clear, 顺序反了会报错
