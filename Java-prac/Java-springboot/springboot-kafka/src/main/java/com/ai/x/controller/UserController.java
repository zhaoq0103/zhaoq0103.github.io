...
import com.ai.x.service.MessagingService;
...
//        User user = (User) session.getAttribute(KEY_USER);
    ...
    public ModelAndView doRegister(@RequestParam("email") String email, @RequestParam("password") String password, @RequestParam("name") String name) {
        try {
            ...
            messagingService.sendRegistrationMessage(RegistrationMessage.of(email,name));
        } catch (Exception e) {
...
//            session.setAttribute(KEY_USER, user);
            ...
            map.put("email", email);
            map.put("error", "Register failed");
            messagingService.sendLoginMessage(LoginMessage.of(email, "(unkown)", false));
