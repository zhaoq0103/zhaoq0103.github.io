...
import com.ai.x.exception.ServerWebInputException;
import com.ai.x.io.PropertyResolver;
import com.ai.x.utils.JsonUtils;
import com.ai.x.web.controller.ApiController;
import com.ai.x.web.controller.ControllerConfiguration;
...
class DispatcherServletTest {
    ...
    void getGreeting() throws ServletException, IOException {
        var req = createMockRequest("GET", "/greeting", null, Map.of("name", "Bob", "nousing","leftithere"));
        var resp = createMockResponse();
        this.dispatcherServlet.service(req, resp);
        assertEquals(200, resp.getStatus());
        assertEquals("Hello, Bob", resp.getContentAsString());
    ...
    void getGreeting3() throws ServletException, IOException {
        ...
        // Exception catched by doService(), will not throw out
...
//        });
    ...
    void getApiDownload() throws ServletException, IOException {
        ...
        assertTrue(resp.getContentAsString().contains("\"content\":\"QUFBQUFBQUE=\"")); // AAAAAAAA  因为是byte[], 进行了base64编码
    }
    ...
    @Test
    void postSigninWithBody() throws ServletException, IOException {
        var req = createMockRequest("POST", "/signinv2", Map.of("name", "Bob", "password", "hello123"),null);
        var resp = createMockResponse();
        this.dispatcherServlet.service(req, resp);
        assertEquals(302, resp.getStatus());
        assertEquals("/home?name=Bob", resp.getRedirectedUrl());
