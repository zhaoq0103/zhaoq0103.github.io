package com.ai.x.context;

import com.ai.x.annotation.*;
import com.ai.x.exception.*;
import com.ai.x.io.PropertyResolver;
import com.ai.x.io.ResourceResolver;
import com.ai.x.utils.ClassUtils;
...
public class AnnotationConfigApplicationContext implements ConfigurableApplicationContext {
    ...
    public AnnotationConfigApplicationContext(Class<?> configClass, PropertyResolver propertyResolver) {
        ...
        // 这个 beanPostProcessors 是有序的 sorted()，是为了后续能够反序 reverse() 还原回来
        /*处理多次代理的情况，这段解释合理吗？有点不合理呢
        即一个原始Bean，比如UserService，被一个事务处理的BeanPostProcsssor代理为UserServiceTx，
        又被一个性能监控的BeanPostProcessor代理为UserServiceMetric，
        还原的时候，对BeanPostProcsssor做一个倒序，
        先还原为UserServiceTx，再还原为UserService。
    ...
    void initBean(BeanDefinition def) {
        ...
        beanPostProcessors.forEach(beanPostProcessor -> {
            ...
            /*
             这个函数一般不处理任何事项的话，那下面的逻辑就不会走，但是有可能会被改写。所以初始化完成后，把proxy设置到instance
            * */
