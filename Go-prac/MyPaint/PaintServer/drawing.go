...
func (p *Drawing) Sync(shapes []ShapeID, changes []Shape) (err error) {
	...
	for _, change := range changes {
		spid := change.GetID()
		// log.Println(" before insert shape,shapeid:", spid)
		// Upsert 总是执行失败，执行测试用例的时候发现是  反射获取字段信息失败，struct的属性大小写影响了
		...
		if err != nil {
			log.Println("shape collection upsert failed ")
			return mgoError(err)
		}
	}
	drawingColl := c.DB(DBName).C("drawing")
	// log.Println(" before insert drawing, dgid:", p.id)
	return mgoError(drawingColl.UpdateId(p.id, M{"$set": M{"shapes": shapes}}))
}
...
func (p *Drawing) Add(shape Shape) (err error) {
	...
	shapeColl := c.DB(DBName).C("shape")
	spid := shape.GetID()
	log.Println("Drawing Add shape spid:", spid)
...
func (p *Document) Add(uid UserID) (drawing *Drawing, err error) {
	...
	drawingColl := c.DB(DBName).C("drawing")
	id := bson.NewObjectId()
	log.Println("Document Add drawing dgid:", id)
